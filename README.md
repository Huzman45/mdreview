...
The value is read from the ASGI scope rather than from configuration, so a proxy
that advertises the prefix itself works without setting the variable at all —
as does `uvicorn --root-path /mdreview`, or mounting the app inside a larger
one. Leading and trailing slashes are optional; `mdreview`, `/mdreview` and
