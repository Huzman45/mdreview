...
## Purpose
The HTTP service lifecycle: where it listens, where it stores state, how its
schema is migrated, and how it is started on demand without a daemon.

## Requirements
### Requirement: Loopback binding by default
The service SHALL bind to the loopback interface by default, because it carries
no authentication of any kind. It MUST NOT listen on any other address unless
the operator explicitly opts in, as specified by `private-lan-access`.
...
#### Scenario: Non-loopback host is refused without an opt-in
- **WHEN** the server is asked to bind to a host that is not a loopback address
  and no LAN opt-in was given
